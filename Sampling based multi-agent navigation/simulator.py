...
"""
    Initialize parameters to run a simulation
"""
...
def readScenario(fileName, scalex=1., scaley=1.):
    ...
    fp.close()
    for line in lines:
        agents.append(Agent(line.split(','),5,2))  # create an agent and add it to the list
    ...
    num = len(agents);
    print("number of agents now =", num)
...
def initWorld(canvas):
    ...
    for a in agents:
        ...
        text.append(canvas.create_text(0, 0, fill="black", font="Calibri 10", text="Agent {}".format(a.id)))
...
def drawWorld():
    ...
    for i in range(len(agents)):
        ...
        if not agent.atGoal:
            ...
            canvas.coords(text[i], world_scale * (agent.pos[0] - world_xmin), world_scale * (agent.pos[1] - world_ymin))
            if drawVels:
